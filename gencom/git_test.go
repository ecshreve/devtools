...
func TestGetGitDiff(t *testing.T) {
	...
	git.Diff = "mocked-diff-output"
	diff, err = git.GetDiff()
	assert.NoError(t, err)
	assert.Equal(t, "mocked-diff-output", diff)

	git.Diff = getDiffHelper()
	diff, err = git.GetDiff()
	assert.NoError(t, err)

	expectedDiff := `diff --git a/internal/cache/tinycache.go b/internal/cache/tinycache.go
...
func getDiffHelper() string {
	log.Info("getDiffHelper")
